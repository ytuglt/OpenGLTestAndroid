...
{
    LOGI("OpenGLSample::LoadImage index=%d, pImage = %p", index, pImage->ppPlane[0]);
    if (pImage)
    {
...
void OpenGLSample::Init() {
    ...
    {
        LOGI("OpenGLSample::Init create program fail");
        return;
    }
...
void OpenGLSample::Draw(int screenW, int screenH) {
    LOGI("OpenGLSample::Draw screenW=%d, screenH=%d", screenW, screenH);
...
void OpenGLSample::UpdateMatrix(glm::mat4 &mvpMatrix, int angleXRotate, int angleYRotate, float scale, glm::vec3 transVec3, float ratio)
{
