...
GLuint GLUtils::CreateProgram(const char *vShaderStr, const char *fShaderStr,
    ...
    if (!vertexShaderHandle) {
        LOGE("GLUtils::CreateProgram vertexShaderHandle error is 0");
        return program;
    }

    fragShaderHandle = LoadShader(GL_FRAGMENT_SHADER, fShaderStr);
    if (!fragShaderHandle) {
        LOGE("GLUtils::CreateProgram fragShaderHandle error is 0");
        return program;
    }

    program = glCreateProgram();
    if (!program) {
        LOGE("GLUtils::CreateProgram program error is 0");
        return program;
    }
    ...
    if (linkStatus != GL_TRUE) {
        ...
        if (bufLength) {
            ...
            if (buf) {
                ...
                LOGE("GLUtils::CreateProgram program link failed, infoLog: %s\n", buf);
                free(buf);
            }
        }
...
GLuint GLUtils::CreateProgramWithFeedback(const char *pVertexShaderSource, const char *pFragShaderSource,
    ...
    FUN_BEGIN_TIME("GLUtils::CreateProgramWithFeedback")
        ...
        if (!vertexShaderHandle) {
            LOGE("GLUtils::CreateProgramWithFeedback vertexShaderHandle error is 0");
            return program;
        }

        fragShaderHandle = LoadShader(GL_FRAGMENT_SHADER, pFragShaderSource);
        if (!fragShaderHandle) {
            LOGE("GLUtils::CreateProgramWithFeedback fragShaderHandle error is 0");
            return program;
        }

        program = glCreateProgram();
        if (!program) {
            LOGE("GLUtils::CreateProgramWithFeedback program error is 0");
            return program;
        }
        glAttachShader(program, vertexShaderHandle);
        CheckGLError("glAttachShader");
        ...
        if (linkStatus != GL_TRUE)  {
            ...
            if (bufLength)
            {
                ...
                {
                    glGetProgramInfoLog(program, bufLength, NULL, buf);
                    LOGI("GLUtils::CreateProgramWithFeedback Could not link program:\n%s\n", buf);
                    free(buf);
                }
            }
            glDeleteProgram(program);
    ...
    LOGI("GLUtils::CreateProgramWithFeedback program: %d", program);
    return program;
}
